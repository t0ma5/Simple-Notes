 ...
 * Default Dark red theme: dark (not black) page, contrasting cards, red status bar
 * Notes is a two-column card grid by default; the toolbar icon switches to notebooks
 * Notebook cards show a note count on the left and a six-dot `:::` handle on the right
 * Lock notebooks and notes with fingerprint, pattern, or PIN
 * Note cards show title plus 4 unwrapped preview lines; long-press to reorder
 * Encrypted or plain exports; another phone can restore them
